...
class PostgresTransactionErrorFault(InfraFault.InfraFault):
    ...
    def prereq_check(self):
        ...
        if not status:
            ...
            if not is_table_exist:
                ...
                if is_trigger_exist:
                    pre_req_error_msg = FaultConstants.DB_TRIGGER_NOT_EXIST.format("error",
                                                                                   self.fault_args.get(
    ...
    def inject_fault(self):
        ...
        try:
            ...
            print("Error while injecting Postgres Db transaction error fault", error)
            self.faultinfo.status = FaultStatus.INJECTION_FAILED.name
            self.clean()
        log.info("Triggered Postgres Db transaction error fault...")
        ...
        try:
            ...
            if conn:
                ...
                query = query.format(percentage=self.fault_args.get(FaultConstants.DB_PERCENTAGE),
                                     errorCode=self.fault_args.get(FaultConstants.DB_ERROR_CODE))
                cursor.execute(query)
                conn.commit()
                status = True

        except (Exception, psycopg2.DatabaseError) as error:
            log.error("Error while create trigger exception function of Postgres %s", error)
        finally:
            if conn:
                cursor.close()
                self.close_connection(conn)
    ...
    def create_trigger(self):
        ...
        try:
            ...
            if conn:
                cursor = conn.cursor()
                query = """
                   CREATE TRIGGER mangleExceptiontrigger
            ...
            log.error("Error while create exception trigger of Postgres %s", error)
        finally:
            if conn:
                cursor.close()
                self.close_connection(conn)
    ...
    def clean(self):
        ...
        try:
            ...
            if conn:
                cursor = conn.cursor()
                query = """
                    DROP TRIGGER mangleExceptiontrigger on {tableName};
            ...
            log.error("Error while cleaning the Postgres Db transaction error fault %s", error)
        finally:
            if conn:
                cursor.close()
                self.close_connection(conn)
...
if __name__ == '__main__':
    fault_args = {'--operation': 'inject', '--faultname': "dbTransactionErrorFault_postgres", "--userName": "postgres",
                  "--password": "vmware", "--port": 5432, "--dbName": "postgres", "--sslEnabled": True,
                  "--tableName": "customer", "--percentage": 75, "--errorCode": 25000,
                  "--timeout": 120000,
                  "--faultId": "1234"}
