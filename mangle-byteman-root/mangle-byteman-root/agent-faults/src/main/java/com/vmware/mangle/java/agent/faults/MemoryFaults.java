 ...
 *
 */
public class MemoryFaults extends AgentFault {

    public MemoryFaults(Map<String, String> faultArgs) throws FiaascoException {
        super(faultArgs);
    }

    @Override
    protected void populateFutureList() {
        futureList.addAll(ThreadUtils.triggerMemoryLoadGenerator(getLong(getFaultArgs().get("timeOutInMilliSeconds")),
                getInt(getFaultArgs().get("load"))));
    }
    ...
    public Map<String, String> getFaultSpecificParams() {
        ...
        params.put("load",
                "Integer value Representing Memory Usage %. Range: 1 to 100 More than 100 Considered as 100.");
        if (isLongLasting()) {
