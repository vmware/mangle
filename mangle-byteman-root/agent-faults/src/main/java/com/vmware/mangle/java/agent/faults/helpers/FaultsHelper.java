...
public class FaultsHelper {
    ...
    private Map<String, ApplicationFault> applicationFaultsMap = new HashMap<String, ApplicationFault>();
    ...
    public String getFault(String id) {
        ...
        if (fault == null) {
            ApplicationFault applicationFault = applicationFaultsMap.get(id);
            return applicationFault != null ? objectToJson(applicationFault.getFaultInfo())
                    : "No fault found with provided ID: " + id;
        }
        return fault != null ? objectToJson(fault.getFaultInfo()) : "No fault found with provided ID: " + id;
    }
    ...
    public Map<String, ApplicationFault> getApplicationFaultsMap() {
        return applicationFaultsMap;
    }

    public Map<String, String> getLiveApplicationFaults() {
        Map<String, String> faultsInfo = new HashMap<>();
        for (ApplicationFault fault : applicationFaultsMap.values()) {
            if (fault.getFaultInfo().getFaultStatus() == FaultStatus.IN_PROGRESS
                    || fault.getFaultInfo().getFaultStatus() == FaultStatus.INITIALIZING) {
                faultsInfo.put(fault.getId(), fault.getFaultInfo().getScriptText());
            }
        }
        return faultsInfo;
    }

    public Map<String, String> getAllApplicationFaults() {
        Map<String, String> faultsInfo = new HashMap<>();
        for (ApplicationFault fault : applicationFaultsMap.values()) {
            faultsInfo.put(fault.getId(), fault.getFaultInfo().getScriptText());
        }
    ...
    public Map<String, ApplicationFaultInfo> getAllApplicationFaultsInfo() {
        Map<String, ApplicationFaultInfo> faultsInfo = new HashMap<>();
        for (ApplicationFault fault : applicationFaultsMap.values()) {
            faultsInfo.put(fault.getId(), fault.getFaultInfo());
        }
        return faultsInfo;
    }
}
