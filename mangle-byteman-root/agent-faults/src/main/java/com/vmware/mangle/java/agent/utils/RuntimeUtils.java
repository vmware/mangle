...
public class RuntimeUtils {
    ...
    public static long getFreeHeapSpace() {
        return getMaxHeapSpace() - getUsedHeapSpace();
    }
    ...
    public static long getMaxUsableHeapSpace() {
        return (runtime.maxMemory() - (HeapInfoUtils.getFromSpace() * 1024));
    }

    public static double getCurrentHeapUsageInPercentage() {
        double value = Math.round(((double) getUsedHeapSpace() * 100) / (double) getMaxUsableHeapSpace());
        if (value > 100) {
            value = 100;
        }
        return value;
