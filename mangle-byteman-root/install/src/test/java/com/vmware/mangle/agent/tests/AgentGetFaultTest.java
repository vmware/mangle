 ...
 *
 */
public class AgentGetFaultTest {
    ...
    @Test
    public void bytemanGetFaultTest() {
        ...
        Submit.main(new String[] { "-gf", faultId.trim() });
        Assert.assertTrue(baos.toString().contains("cpuFault-" + faultId), baos.toString());
        try {
            baos.flush();
        } catch (IOException e) {
            e.printStackTrace();
        ...
        Assert.assertTrue(
                baos.toString().contains("No fault found with provided ID: 267be1f0-08f9-48bc-97f0-b92b02c68950"),
                baos.toString());
        try {
            baos.flush();
        } catch (IOException e) {
