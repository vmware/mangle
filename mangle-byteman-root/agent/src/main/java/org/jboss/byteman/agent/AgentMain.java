...
public class AgentMain {
    ...
    public static void premain(String args, Instrumentation inst) throws Exception {
        ...
        synchronized (AgentMain.class) {
            if (!firstTime || !Boolean.getBoolean(BYTEMAN_LISTENER_LOADED)) {
                firstTime = true;
                System.setProperty(BYTEMAN_AGENT_LOADED, Boolean.TRUE.toString());
            } else {
                throw new Exception("Main : attempting to load Byteman Listener more than once");
            }
        }
        ...
        for (String scriptPath : resourcescriptPaths) {
            try {
                ...
                if (is == null) {
                    throw new Exception(
                            "org.jboss.byteman.agent.Main: could not read rule script resource file : " + scriptPath);
                }
                ...
                System.err.println(
                        "org.jboss.byteman.agent.Main: error reading rule script resource file : " + scriptPath);
                throw ioe;
            }
        }
        ...
        if (managerClassName != null && isRedefine) {
            ...
            Constructor/*<Transformer>*/ constructor = transformerClazz.getConstructor(Instrumentation.class,
                    moduleSystemInteraceClazz, List.class, List.class, boolean.class);
            transformer = (ClassFileTransformer) constructor
                    .newInstance(new Object[] { inst, moduleSystem, scriptPaths, scripts, isRedefine });
        } else {
            ...
            Constructor/*<Retransformer>*/ constructor = transformerClazz.getConstructor(Instrumentation.class,
                    moduleSystemInteraceClazz, List.class, List.class, boolean.class);
            transformer = (ClassFileTransformer) constructor
                    .newInstance(new Object[] { inst, moduleSystem, scriptPaths, scripts, isRedefine });
        }
        ...
        if (managerClassName != null && isRedefine) {
            ...
            System.setProperty(BYTEMAN_LISTENER_LOADED, Boolean.TRUE.toString());
        }
    ...
    /**
     * prefix used to request installation of an access-all-areas security policy at install time
     * for agent code
     */
    ...
    private static final String MANAGER_NAME = "org.jboss.byteman.agent.FiaascoTransformListener";
    ...
    /**
     * The name of the module system implementation class, supplied on the command line (optional
     * argument)
     */
    private static String moduleSystemName = null;

    /**
     * The arguments to the module system implementation class, supplied on the command line
     * (optional argument)
     */
