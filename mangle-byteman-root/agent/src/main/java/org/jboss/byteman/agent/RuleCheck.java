...
public class RuleCheck {
    ...
    public void checkRules() {
        ClassLoader loader = RuleCheck.class.getClassLoader().getSystemClassLoader();
        ...
        for (RuleScript script : allScripts) {
            ...
            if (targetClass == null && targetClassName.indexOf('.') < 0) {
                for (int i = 0; i < packages.toArray().length; i++) {
                    ...
                    try {
                        ...
                        error("ERROR : Unexpected error looking up " + targetClassName + " in package "
                                + packages.toArray()[i], e);
                        return;
                    }
                    if (targetClass != null) {
                        break;
            ...
            if (targetClass == null) {
                error("ERROR : Could not load class " + targetClassName + " declared in rule \"" + script.getName()
                        + "\" loaded from " + script.getFile() + " line " + script.getLine());
                continue;
            }

            // make sure it is the right type of class
            if (script.isInterface() && !targetClass.isInterface()) {
                error("ERROR : Found class instead of interface for rule \"" + script.getName() + "\" loaded from "
                        + script.getFile() + " line " + script.getLine());
                continue;
            }

            if (!script.isInterface() && targetClass.isInterface()) {
                error("ERROR : Found interface instead of class for rule \"" + script.getName() + "\" loaded from "
                        + script.getFile() + " line " + script.getLine());
                continue;
            }
            ...
            if (!script.isInterface()) {
                ...
                try {
                    ...
                    if (read < max) {
                        error("ERROR : Could not load bytecode for class " + targetClassName + " declared in rule \""
                                + script.getName() + "\" loaded from " + script.getFile() + " line "
                                + script.getLine());
                        continue;
                    }
                } catch (Exception e) {
                    error("ERROR : Could not load bytecode for class " + targetClassName + " declared in rule \""
                            + script.getName() + "\" loaded from " + script.getFile() + " line " + script.getLine(), e);
                    continue;
                }
                ...
                try {
                    transformer = new Transformer(null, helperManager.getModuleSystem(), emptyInitialTexts,
                            emptyInitialFiles, false);
                } catch (Exception e) {
                    // will not happen!
                }
            ...
            if (script.hasTransform(targetClass)) {
                ...
                for (Transform transform : transforms) {
                    ...
                    if (throwable != null) {
                        if (throwable instanceof ParseException) {
                            parseError("ERROR : Failed to parse rule \"" + script.getName() + "\" loaded from "
                                    + script.getFile() + " line " + script.getLine(), throwable);
                        } else if (throwable instanceof TypeWarningException) {
                            typeWarning("WARNING : Problem type checking rule \"" + script.getName() + "\" loaded from "
                                    + script.getFile() + " line " + script.getLine()
                                    + (methodName == null ? "" : " against method " + methodName), throwable);
                        } else if (throwable instanceof TypeException) {
                            typeError("ERROR : Failed to type check rule \"" + script.getName() + "\" loaded from "
                                    + script.getFile() + " line " + script.getLine()
                                    + (methodName == null ? "" : " against method " + methodName), throwable);
                        } else {
                            error("ERROR : Unexpected exception transforming class " + targetClassName
                                    + " using  rule \"" + script.getName() + "\" loaded from " + script.getFile()
                                    + " line " + script.getLine()
                                    + (methodName == null ? "" : " against method " + methodName), throwable);
                        }
                        continue;
                    ...
                    infoMessage =
                            "Parsed rule \"" + script.getName() + "\" for class " + transform.getInternalClassName();
                    ...
                    try {
                        ...
                        typeWarning("WARNING : Unable to type check rule \"" + script.getName() + "\" loaded from "
                                + script.getFile() + " line " + script.getLine()
                                + (methodName == null ? "" : " against method " + methodName), te);
                        continue;
                        ...
                        typeError("ERROR : Failed to compile rule \"" + script.getName() + "\" loaded from "
                                + script.getFile() + " line " + script.getLine()
                                + (methodName == null ? "" : " against method " + methodName), ce);
                        continue;
                    } catch (Throwable th) {
                        typeError("ERROR : Failed to check rule \"" + script.getName() + "\" loaded from "
                                + script.getFile() + " line " + script.getLine()
                                + (methodName == null ? "" : " against method " + methodName), th);
                        continue;
                    }

                    if (script.isOverride()) {
                        info("Type checked overriding rule \"" + script.getName()
                                + "\" against method in declared class");
                    } else {
                ...
                try {
                    ...
                    parseError("ERROR : Failed to type check rule \"" + script.getName() + "\" loaded from "
                            + script.getFile() + " line " + script.getLine(), pe);
                    continue;
                } catch (TypeWarningException te) {
                    typeWarning("WARNING : Unable to type check rule \"" + script.getName() + "\" loaded from "
                            + script.getFile() + " line " + script.getLine(), te);
                    continue;
                } catch (TypeException te) {
                    typeError("ERROR : Failed to type check rule \"" + script.getName() + "\" loaded from "
                            + script.getFile() + " line " + script.getLine(), te);
                    ...
                    error("ERROR : Failed to process rule \"" + script.getName() + "\" loaded from " + script.getFile()
                            + " line " + script.getLine(), th);
                    continue;
                }
                ...
                warning("WARNING : Unable to transform class " + targetClassName + " using rule \"" + script.getName()
                        + "\" loaded from " + script.getFile() + " line " + script.getLine());
            }
            info("");
        }
    ...
    private void typeCheckAgainstMethodDeclaration(Rule rule, RuleScript script, Class targetClass,
            ClassLoader loader) {
        ...
        if (targetName == "<clinit>") {
            warning("WARNING : Cannot type check <clinit> rule \"" + script.getName() + "\" loaded from "
                    + script.getFile() + " line " + script.getLine());
            return;
        }
        if (targetMethodName == "<init>") {
            ...
            if (script.isInterface()) {
                error("ERROR : Invalid target method <init> for interface rule \"" + script.getName()
                        + "\" loaded from " + script.getFile() + " line " + script.getLine());
                return;
            } else {
                error("ERROR : Invalid target method <init> for overriding rule \"" + script.getName()
                        + "\" loaded from " + script.getFile() + " line " + script.getLine());
                return;
            }
        } else {
            ...
            for (Method candidate : candidates) {
                ...
                if (targetName.equals(candidateName)
                    ...
                    int paramErrorCount =
                            installParamTypes(rule, targetClass.getName(), access, candidateName, candidateDesc);
                    if (paramErrorCount == 0) {
                        try {
                            ...
                            typeError("ERROR : Failed to compile rule \"" + script.getName() + "\" loaded from "
                                    + script.getFile() + " line " + script.getLine(), ce);
                            System.out.println(ce);
                            System.out.println();
                            return;
                        } catch (Throwable th) {
                            error("ERROR : Failed to process rule \"" + script.getName() + "\" loaded from "
                                    + script.getFile() + " line " + script.getLine(), th);
                            return;
                        }
                        if (script.isInterface()) {
                            ...
                            info("Type checked overriding rule \"" + script.getName()
                                    + "\" against method declaration");
                        }
                    } else {
                        info("Failed to type check rule \"" + script.getName() + "\" loaded from " + script.getFile()
                                + " line " + script.getLine());
                    }
                }
            }
    ...
    public int installParamTypes(Rule rule, String targetClassName, int access, String candidateName,
        ...
        while (iterator.hasNext()) {
            ...
            if (binding.getType() == Type.UNDEFINED) {
                if (binding.isRecipient()) {
                    ...
                    warning("WARNING : Cannot typecheck local variable " + binding.getName() + " in rule \""
                            + rule.getName() + "\"");
                    binding.setDescriptor("void");
                }
            }
