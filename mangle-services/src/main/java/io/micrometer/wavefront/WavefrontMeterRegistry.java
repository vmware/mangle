...
public class WavefrontMeterRegistry extends StepMeterRegistry {
    ...
    /**
     * @param config
     *            Configuration options for the registry that are describable as properties.
     * @param clock
     ...
     *            The clock to use for timings.
     * @param threadFactory
     *            The thread factory to use to create the publishing thread.
    ...
    protected void publish() {
        ...
        if (StringUtils.isEmpty(config.uri())) {
            // We cannot send metrics when the URI of the metric provider is empty. Hence returning without executing remaining code
            return;
        }
        for (List<Meter> batch : MeterPartition.partition(this, config.batchSize())) {
            Stream<String> stream = batch.stream()
                    .flatMap(m -> m.match(this::writeMeter, this::writeMeter, this::writeTimer, this::writeSummary,
                            this::writeMeter, this::writeMeter, this::writeMeter, this::writeFunctionTimer,
                            this::writeMeter));

            if (directToApi()) {
                try {
                    httpClient.post(config.uri() + "/report/metrics?t=" + config.apiToken() + "&h=" + config.source())
                            .acceptJson().withJsonContent("{" + stream.collect(joining(",")) + "}").send()
                            .onSuccess(response -> logSuccessfulMetricsSent(batch)).onError(response -> logger
                                    .error("failed to send metrics to wavefront: {}", response.body()));
                } catch (Throwable e) {
    ...
    private Stream<String> writeMeter(Meter meter) {
        ...
        stream(meter.measure().spliterator(), false).forEach(measurement -> {
    ...
    private String writeMetric(Meter.Id id, @Nullable String suffix, long wallTime, double value) {
        return directToApi() ? writeMetricDirect(id, suffix, value) : writeMetricProxy(id, suffix, wallTime, value);
    }
    ...
    private String writeMetricProxy(Meter.Id id, @Nullable String suffix, long wallTime, double value) {
        ...
        return "\"" + getConventionName(fullId) + "\" " + DoubleFormat.decimalOrNan(value) + " " + (wallTime / 1000)
                + " source=" + config.source() + " " + getConventionTags(fullId).stream()
    ...
    private String writeMetricDirect(Meter.Id id, @Nullable String suffix, double value) {
        ...
        String tags = conventionTags.stream()
        ...
        return "\"" + escapeJson(getConventionName(fullId)) + "$" + uniqueNameSuffix + "\"" + ": {" + "\"value\": "
                + DoubleFormat.decimalOrNan(value) + "," + "\"tags\": {" + tags + "}" + "}";
    }
