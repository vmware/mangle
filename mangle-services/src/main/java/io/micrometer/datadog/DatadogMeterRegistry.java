...
public class DatadogMeterRegistry extends StepMeterRegistry {
            ...
            for (List<Meter> batch : MeterPartition.partition(this, config.batchSize())) {
                ...
                */
                String body = batch.stream()
                        .flatMap(meter -> meter.match(m -> writeMeter(m, metadataToSend),
                                m -> writeMeter(m, metadataToSend), timer -> writeTimer(timer, metadataToSend),
                                summary -> writeSummary(summary, metadataToSend), m -> writeMeter(m, metadataToSend),
                                m -> writeMeter(m, metadataToSend), m -> writeMeter(m, metadataToSend),
                                timer -> writeTimer(timer, metadataToSend), m -> writeMeter(m, metadataToSend)))
    ...
    private Stream<String> writeTimer(FunctionTimer timer, Map<String, DatadogMetricMetadata> metadata) {
        ...
        return Stream.of(writeMetric(id, "count", wallTime, timer.count()),
    ...
    String writeMetric(Meter.Id id, @Nullable String suffix, long wallTime, double value) {
        ...
        String host = config.hostTag() == null ? ""
                : stream(tags.spliterator(), false).filter(t -> requireNonNull(config.hostTag()).equals(t.getKey()))
                        .findAny().map(t -> ",\"host\":\"" + escapeJson(t.getValue()) + "\"").orElse("");

        String tagsArray =
                tags.iterator().hasNext()
