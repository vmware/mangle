 ...
 *
 */
public class MetricProviderHelperTest {
    ...
    @Test(description = "Validation when no active metric providers are defined")
    public void testWhenNoActiveMetricProvider() throws MangleException {
        when(metricProviderService.getActiveMetricProvider()).thenReturn(null);
        Notifier notifier = metricProvider.getActiveNotificationProvider();

        Assert.assertNull(notifier);
    }
