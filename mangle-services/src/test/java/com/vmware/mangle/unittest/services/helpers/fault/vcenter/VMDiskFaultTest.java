...
public class VMDiskFaultTest {
    ...
    public void testVMNicFault() throws MangleException {
        VMNicFaultSpec nicFaultSpec = faultsMockData.getVMNicFaultSpec();
        EndpointSpec endpointSpec = endpointMockData.getVCenterEndpointSpecMock();
        nicFaultSpec.setEndpoint(endpointSpec);
        VMNicFault vmNicFault = new VMNicFault(nicFaultSpec);

        Assert.assertNotNull(vmNicFault);
    }
