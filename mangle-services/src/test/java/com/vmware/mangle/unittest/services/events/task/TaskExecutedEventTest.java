...
import com.vmware.mangle.services.events.task.TaskExecutedEvent;
...
public class TaskExecutedEventTest {
    ...
    public void init() {
        ...
        task.setId("12345");
        eventMesage = "Executed Task: " + task.getClass().getName() + " With Id: " + task.getId();
    }


    @Test
    public void lombokShouldSetCorrectly() {
        TaskExecutedEvent<?> event = new TaskExecutedEvent(task);

        Assert.assertEquals(getField(event, "name"), "TaskExecutedEvent");
    ...
    public void lombokShouldGetCorrectly() {
        ...
        Assert.assertEquals(event.getName(), "TaskExecutedEvent");
    ...
    public void eventsShouldNotBeEqualWithTheSameIdButDifferentNames() {
        TaskExecutedEvent<?> event3 = new TaskExecutedEvent(task);
        TaskExecutedEvent<?> event4 = new TaskExecutedEvent(task);
        event4.setName("TaskExecutedEventModified");
    ...
    @Test
    public void testTaskExecutedEventResolvableType() {
        TaskExecutedEvent<?> event = new TaskExecutedEvent(task);
        event.getResolvableType();
        Assert.assertEquals(event.getResolvableType().toString(),
                TaskExecutedEvent.class.getName() + "<" + Event.class.getName() + ">");
    }
}
