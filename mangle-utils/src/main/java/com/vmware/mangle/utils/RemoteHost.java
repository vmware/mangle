...
public class RemoteHost {
	 ...
	 *         <p>
	 *         This function executes a command on the remote host and returns the
	 *         output. If verbose is set, prints the output to stdout
	 */
		...
		try {
			...
			while (true) {
				...
				if (stdErrorBufferedReader.ready()) {
					...
					if (outStream == null && !StringUtils.isEmpty(errStream)) {
						output.append(errStream);
					}
					log.error(errStream);
				}
	 ...
	 * @param userName UserName to login
	 * @param password Password to login
	 * @param src      The file to scp on the remote host
	 * @param dst      The local File where it will be copied
	...
	/**
	 * The logic to support "keyboard-interactive" authentication method. For MN,
	 * "password" mode authentacion for of SSH is not supported. As such, another
	 * mode of authentication: "keyboard-interactive" has to be used. With this
	 * method, a class of type: InteractiveCallBack has to be created so that when
	 * try to connect via SSH to a host/VM, the connection.authenticate function can
	 * call the function: replyToChallenge in InteractiveCallBack class get
	 * responses to the challenges presented.
	 */
