...
public class RestTemplateWrapper {
    ...
    private ResponseEntity<?> execute(Class<?> clz) {
        ...
        try {
            log.debug("URL - " + getHttpRequestMethod().toString() + " - " + getRequestUrl());
    ...
    public static <T> T jsonToObject(String json, @SuppressWarnings("rawtypes") TypeReference object) {
        try {
            return new ObjectMapper().readValue(json, object);
        } catch (JsonSyntaxException jse) {
            log.error("Input String is not a valid json" + jse.getMessage());
