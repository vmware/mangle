...
@Component({
  selector: "app-simulatejavaexception",
  templateUrl: "./simulatejavaexception.component.html"
})
export class SimulateJavaExceptionComponent extends FaultCommons implements OnInit {
  ...
  public executeSimulateJavaExceptionFault(faultData) {
    this.runBtnState = ClrLoadingState.LOADING;
    if (this.tagsData !== {}) {
      faultData.tags = this.tagsData;
    }
    this.addNotifiersInFault(faultData);
    this.faultService.executeSimulateJavaExceptionFault(faultData).subscribe(
      res => {
        this.tagsData = {};
