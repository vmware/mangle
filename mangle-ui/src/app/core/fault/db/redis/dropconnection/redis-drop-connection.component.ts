...
@Component({
  selector: "app-db-redis-drop-connection",
  templateUrl: "./redis-drop-connection.component.html"
})
export class RedisDbDropConnectionComponent extends FaultCommons implements AfterViewInit {
  ...
  public executeRedisDropConnectionFault(faultData: any) {
    this.runBtnState = ClrLoadingState.LOADING;
    if (this.tagsData !== {}) {
      faultData.tags = this.tagsData;
    }
    this.addNotifiersInFault(faultData);
    this.faultService.executeRedisDropConnectionFault(faultData).subscribe(
      res => {
        this.tagsData = {};
