...
@Component({
  selector: "app-db-redis-return-empty",
  templateUrl: "./redis-return-empty.component.html"
})
export class RedisDbReturnEmptyComponent extends FaultCommons implements AfterViewInit {
  ...
  public executeRedisReturnEmptyFault(faultData: any) {
    this.runBtnState = ClrLoadingState.LOADING;
    if (this.tagsData !== {}) {
      faultData.tags = this.tagsData;
    }
    this.addNotifiersInFault(faultData);
    this.faultService.executeRedisReturnEmptyFault(faultData).subscribe(
      res => {
        this.tagsData = {};
