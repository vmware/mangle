...
@Component({
  selector: "app-db-redis-return-error",
  templateUrl: "./redis-return-error.component.html"
})
export class RedisDbReturnErrorComponent extends FaultCommons implements AfterViewInit {

  public supportedEpTypes: any = [CommonConstants.REDIS_FI_PROXY];
  public errorTypes: any = ["CON", "ERR"];
  ...
  public populateFaultData() {
    this.faultFormData.errorType = this.dataService.sharedData.errorType;
  ...
  public executeRedisReturnErrorFault(faultData: any) {
    this.runBtnState = ClrLoadingState.LOADING;
    if (this.tagsData !== {}) {
      faultData.tags = this.tagsData;
    }
    this.addNotifiersInFault(faultData);
    this.faultService.executeRedisReturnErrorFault(faultData).subscribe(
      res => {
        this.tagsData = {};
