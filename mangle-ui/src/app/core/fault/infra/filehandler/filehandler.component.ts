...
@Component({
  selector: "app-filehandler-infra",
  templateUrl: "./filehandler.component.html"
})
export class FilehandlerInfraComponent extends FaultCommons implements OnInit {
  ...
  public executeFilehandlerLeakFault(faultData) {
    this.runBtnState = ClrLoadingState.LOADING;
    if (this.tagsData !== {}) {
      faultData.tags = this.tagsData;
    }
    this.addNotifiersInFault(faultData);
    this.faultService.executeFilehandlerLeakFault(faultData).subscribe(
      res => {
        this.tagsData = {};
